...
export class BbPage {
  ...
  async goto(url, _opts = {}) {
    // If we already have a tab open, try to close it first to avoid tab explosion
    if (this._tabId) {
      try {
        bb('close', '--tab', this._tabId);
        this._openedTabs = this._openedTabs.filter(id => id !== this._tabId);
        this._tabId = null;
      } catch (e) {
        // console.warn(`Failed to close previous tab ${this._tabId}: ${e.message}`);
      }
    }
