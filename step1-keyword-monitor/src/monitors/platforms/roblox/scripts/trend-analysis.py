...
# Ludusdex 级过滤参数
LU_MAX_AGE_DAYS = 180      # 上线时间 < 180 天
LU_MIN_PLAYERS = 10000     # 并发 > 10K
LU_MIN_RATING = 90.0       # 评分 > 90%
LU_TARGET_GENRES = ["Simulator", "Tycoon", "Tower Defense", "RPG", "Strategy", "Fighting", "Incremental Simulator"]

def _age_days(created_at: str | None, today_utc: str) -> int | None:
    if not created_at:
        return None
    try:
        # 处理可能的不同日期格式
    ...
    for r in conn.execute(q, (t24, t72, t7d)):
        rows.append(
            GameRow(
                ...
                genre=str(r[8]) if r[8] is not None else None,
...
def classify(g: GameRow, today_utc: str):
    # 优先使用真正的创建日期 (created_at)，如果没有则退而求其次使用监控首次发现日期 (first_date)
    age = _age_days(g.created_at or g.first_date, today_utc)
    is_fresh = age is not None and age <= FRESH_GAME_DAYS
    is_new = age is not None and age <= NEW_GAME_DAYS
    
    # Ludusdex 级核心判断
    is_ludusdex_gold = False
    if age is not None and age <= LU_MAX_AGE_DAYS:
        if g.avg_24h >= LU_MIN_PLAYERS:
            if g.rating is None or g.rating >= LU_MIN_RATING:
                if g.genre:
                    for target in LU_TARGET_GENRES:
                        if target.lower() in g.genre.lower():
                            is_ludusdex_gold = True
                            break
    ...
    # 黄金候选（Ludusdex 逻辑）
    if is_ludusdex_gold:
        return "黄金候选", is_new, is_fresh, age
...
def main():
    ...
    # Top10 建站清单：优先 黄金候选/爆发/小盘起飞/新游戏，再按分数
    candidates = [x for x in scored if x[2] in ("黄金候选", "爆发", "小盘起飞", "大盘回暖") or x[3]]
    candidates.sort(key=lambda x: (x[2] == "黄金候选", x[0], x[1].avg_24h), reverse=True)
    top10 = candidates[:10]
        ...
        buildlist.append(
            {
                ...
                "rating": g.rating,
                "age_days": age,
                "reason": f"{tag} | vs72h {g.pct_vs72h():+.1f}% | Genre: {g.genre}",
            }
        )
    ...
    def fmt_line(i, s, g: GameRow, bucket, is_new, is_fresh):
        ...
        age_txt = f" (上线 {_age_days(g.created_at or g.first_date, today_utc)}d)" if (g.created_at or g.first_date) else ""
        
        return (
            f"{i}. **{g.name}** {flag_txt}{age_txt} ⭐{s}\n"
            f"   👥 24h均值: {g.avg_24h:,} | 峰值: {g.peak_24h:,} | 评分: {g.rating or '?'}\n"
            f"   📊 类型: {g.genre or 'Unknown'} | vs72h: {g.pct_vs72h():+.1f}%\n"
        )
